...
export default function decorate(block) {
  ...
  header.classList.add('main-header', 'with-marquee', 'solid'); // Do not add nav-up initially (Rule 19)
  ...
  navigationItems.forEach((row) => {
    ...
    if (hierarchyRoot) {
      ...
      // Preserve original HTML structure for left-div if it exists in the richtext
      const tempDiv = document.createElement('div');
      moveInstrumentation(hierarchyCell, tempDiv);
      tempDiv.innerHTML = hierarchyCell.innerHTML;
      const originalLeftDiv = tempDiv.querySelector('.left-div');
      if (originalLeftDiv) {
        centerDiv.append(originalLeftDiv);
      }

      const subNavWrap = document.createElement('div');
      subNavWrap.classList.add('sub-nav-wrap');
      centerDiv.append(subNavWrap);
      // Append the hierarchyRoot (the <ul> from the richtext cell) to subNavWrap
      if (hierarchyRoot) {
  ...
  const createIconNav = (isMobile) => {
    ...
    mailLink.href = contactLinkItems[0]?.querySelector('a')?.href || '#';
    if (contactLinkItems[0]) {
      // Use the authored label from the first cell of the contact link item
      mailLink.textContent = contactLinkItems[0].children[0]?.textContent.trim() || 'Contact Us';
      moveInstrumentation(contactLinkItems[0], mailLink);
    } else {
      mailLink.textContent = 'Contact Us';
    }

    // SVG for desktop, text for mobile
    if (!isMobile) {
      ...
      `;
      mailLink.replaceChildren(mailSvg); // Replace text with SVG
    }
    ...
    searchLink.href = '#'; // Search functionality is JS-driven, no direct link
    const searchLensSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    searchLensSvg.setAttribute('viewBox', '0 0 21 21');
    searchLensSvg.setAttribute('fill', 'none');
    searchLensSvg.classList.add('lens');
    searchLensSvg.innerHTML = `
    ...
    `;
    const searchCloseSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    searchCloseSvg.setAttribute('viewBox', '0 0 50 50');
    searchCloseSvg.classList.add('close');
    searchCloseSvg.innerHTML = `
      <path d="M 9.15625 6.3125 L 6.3125 9.15625 L 22.15625 25 L 6.21875 40.96875 L 9.03125 43.78125 L 25 27.84375 L 40.9375 43.78125 L 43.78125 40.9375 L 27.84375 25 L 43.6875 9.15625 L 40.84375 6.3125 L 25 22.15625 Z"></path>
    `;
    searchLink.append(searchLensSvg, searchCloseSvg);
    if (isMobile) {
    ...
    const searchInputSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    searchInputSvg.setAttribute('viewBox', '0 0 21 21');
    searchInputSvg.setAttribute('fill', 'none');
    searchInputSvg.innerHTML = `
      <path d="M15.0934 2.73157L15.0934 2.73156C11.6883 -0.67354 6.14543 -0.67354 2.74033 2.73156C-0.666039 6.13793 -0.666063 11.6795 2.74035 15.0847C4.38993 16.7342 6.58308 17.6433 8.91623 17.6433C10.9916 17.6433 12.9533 16.9181 14.5221 15.5975L19.5217 20.5972C19.6721 20.7476 19.8687 20.8212 20.0632 20.8212C20.2588 20.8212 20.4554 20.7476 20.6059 20.5972C20.905 20.2981 20.905 19.8121 20.6059 19.513L15.6062 14.5132C18.4815 11.0845 18.3159 5.95535 15.0934 2.73157ZM14.0092 14.0004C12.6491 15.3606 10.8404 16.1098 8.91623 16.1098C6.99211 16.1098 5.18468 15.3606 3.82452 14.0004C1.01633 11.1923 1.01633 6.62394 3.82452 3.81575C5.22857 2.41171 7.07147 1.71024 8.91623 1.71024C10.7609 1.71024 12.6052 2.41296 14.0092 3.81575C16.8174 6.62394 16.8174 11.1923 14.0092 14.0004Z" stroke-width="0.25"></path>
    `;
    searchIconDiv.append(searchInputSvg);
    ...
    `;
    submitButton.append(submitLabel, submitSvg);
    ...
    // Replace hardcoded search suggestions with empty divs to avoid double rendering
    // These should be populated dynamically or come from authored content if needed.
