...
export default function decorate(block) {
  ...
  const addressRow = children[2];
  const emailLinkRow = children[3];
  const mapEmbedRow = children[4];
  const formNamePlaceholderRow = children[5];
  const formEmailPlaceholderRow = children[6];
  const formMessagePlaceholderRow = children[7];
  const formSubmitLabelRow = children[8];

  const socialLinkRows = children.slice(9);
  ...
  mainHeading.textContent = mainHeadingRow.children[0]?.textContent.trim() || '';
  ...
  addressDiv.classList.add('d-flex', 'align-items-center');
  const addressParagraph = document.createElement('p');
  addressParagraph.style.paddingLeft = '19px'; // Hardcoded style from original HTML
  addressParagraph.textContent = addressRow.children[0]?.textContent.trim() || '';
  moveInstrumentation(addressRow, addressParagraph);
  addressDiv.append(addressParagraph);
  contactDetailsCol.append(addressDiv);
  ...
  if (foundEmailLink) {
    ...
    emailLink.textContent = foundEmailLink.href.replace('mailto:', ''); // Display actual email, not raw href
  }
  ...
  socialLinkRows.forEach((row) => {
    ...
    if (foundSocialLink) {
      ...
      // Add appropriate SVG based on href
  ...
  const foundMapLink = mapEmbedRow.children[0]?.querySelector('a');
  if (foundMapLink) {
  ...
  hiddenInput.value = ''; // Value from original HTML
  contactForm.append(hiddenInput);

  const nameInput = document.createElement('input');
  nameInput.setAttribute('placeholder', formNamePlaceholderRow.children[0]?.textContent.trim() || 'Name');
  ...
  emailInput.setAttribute('placeholder', formEmailPlaceholderRow.children[0]?.textContent.trim() || 'Email');
  ...
  messageTextarea.setAttribute('placeholder', formMessagePlaceholderRow.children[0]?.textContent.trim() || 'Write Your Message');
  ...
  submitButton.textContent = formSubmitLabelRow.children[0]?.textContent.trim() || 'Submit Your Message';
