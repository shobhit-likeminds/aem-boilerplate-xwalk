...
export default function decorate(block) {
  ...
  colLeft.classList.add('col-lg-6', 'col-12', 'contact-details');
  rowContact.append(colLeft);

  // Address
  const addressWrapper = document.createElement('div');
  addressWrapper.classList.add('d-flex', 'align-items-center');
  colLeft.append(addressWrapper);

  const addressIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  addressIcon.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
  addressIcon.setAttribute('width', '16');
  addressIcon.setAttribute('height', '16');
  addressIcon.setAttribute('fill', 'currentColor');
  addressIcon.classList.add('bi', 'bi-geo-alt');
  addressIcon.setAttribute('viewBox', '0 0 16 16');
  addressIcon.innerHTML = `<path d="M12.166 8.94c-.524 1.062-1.234 2.12-1.96 3.07A31.493 31.493 0 0 1 8 14.58a31.481 31.481 0 0 1-2.206-2.57c-.726-.95-1.436-2.008-1.96-3.07C3.304 7.867 3 6.862 3 6a5 5 0 0 1 10 0c0 .862-.305 1.867-.834 2.94zM8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10z"/><path d="M8 8a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 1a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>`;
  addressWrapper.append(addressIcon);

  const addressText = document.createElement('p');
  addressText.style.paddingLeft = '19px';
  addressText.textContent = addressRow.textContent.trim();
  moveInstrumentation(addressRow, addressText);
  addressWrapper.append(addressText);

  // Email
  const emailWrapper = document.createElement('div');
  colLeft.append(emailWrapper);
  ...
  // The email label comes from emailLabelRow, not emailLinkRow
  emailLink.textContent = emailLabelRow.textContent.trim();
  moveInstrumentation(emailLinkRow, emailLink);
  moveInstrumentation(emailLabelRow, emailLink); // Move instrumentation for the label row too
  ...
  colLeft.append(socialAbout);

  socialLinkRows.forEach((row) => {
    // For social-link-item, the model has one field: "link" (type=aem-content)
    ...
    socialSvg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    socialSvg.setAttribute('width', '16');
    socialSvg.setAttribute('height', '16');
    socialSvg.setAttribute('fill', 'currentColor');
    socialSvg.classList.add('bi');
    socialSvg.setAttribute('viewBox', '0 0 16 16');
    socialSvg.innerHTML = socialIconPath;
    socialAnchor.append(socialSvg);
  ...
  moveInstrumentation(mapEmbedRow, mapEmbed);
  colLeft.append(mapEmbed);

  // Contact Form
  const colRight = document.createElement('div');
  colRight.classList.add('col-lg-6', 'col-12');
  rowContact.append(colRight);

  const contactForm = document.createElement('form');
  contactForm.classList.add('contact-form', 'd-flex', 'flex-column', 'justify-content-around');
  colRight.append(contactForm);

  const hiddenInput = document.createElement('input');
  hiddenInput.setAttribute('hidden', '');
  hiddenInput.setAttribute('name', 'next');
  hiddenInput.setAttribute('value', ''); // Value should come from model if available
