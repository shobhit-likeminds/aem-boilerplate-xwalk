  ...
  slideRows.forEach((row) => {
    ...
    if (desktopImg) {
      const img = createOptimizedPicture(
        desktopImg.src,
        imageAltCell?.textContent.trim() || desktopImg.alt,
        false,
        [{ width: '1903' }],
      );
    ...
    const preheadline = document.createElement('small');
    preheadline.style.fontWeight = 'bold';
    preheadline.textContent = preheadlineCell?.textContent.trim() || '';
    if (preheadline.textContent) contentDiv.append(preheadline);

    const headline = document.createElement('h2');
    headline.classList.add('heading', 'font-medium', 'font-size-tb');
    headline.innerHTML = headlineCell?.innerHTML || '';
    if (headline.innerHTML) contentDiv.append(headline);

    const description = document.createElement('p');
    description.innerHTML = descriptionCell?.innerHTML || '';
    if (description.innerHTML) contentDiv.append(description);
